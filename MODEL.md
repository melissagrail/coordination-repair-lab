...
A single evidence draw is deliberately shared across checks within each opportunity. Checks are therefore correlated. Its threshold is `evidence_accuracy`; despite the name, it models **sensitivity with no false positives**, not a calibrated classifier's overall accuracy. Audits detect true breaches, diagnoses identify true accidents, appeals clear false allegations, and scope checks detect true externalities when that draw is below the threshold. On failure, they detect nothing. This strong simplification must remain visible.
...
4. If the rule screens scope, pay the scope cost and decline a detected harmful task.
...
6. An evidenced breach is rejected before production. This produces no output. Rules with exclusion also reset history and exclude the provider for its next three opportunities.
7. Otherwise pay production cost 1. A normal task succeeds. A failed task under a retry rule incurs diagnosis cost 0.3; an evidenced accident incurs retry cost 1 and succeeds with probability 0.8. Breaches cannot be repaired. Failed diagnosis and failed retries still cost resources.
...
- Externalities are modeled as a known nonnegative loss on completion; there is no measurement uncertainty beyond detection, delayed harm, affected-party discovery, or disagreement over valuation.
