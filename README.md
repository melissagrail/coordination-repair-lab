...
This is a **v0.2 model and reproducibility harness**, not an alignment solution, security product, or demonstrated cooperation attractor. It runs fixed mathematical rules, not language-model agents. No accounts, API keys, network access, or paid compute are needed to run it.

## New in v0.2: checks can be wrong

The [imperfect-evidence study](results/robustness/REPORT.md) adds **31,200 runs across 540 parameter cells**, independently checked payoff formulas, false accusations by verification, unnecessary scope refusals, and wasted retries. The [study plan](studies/ROBUSTNESS_PLAN.md) was committed before running it. All original baseline metrics remain unchanged when false-positive rates are zero.

A useful counterexample: with 5% harmful tasks and the documented costs, scope screening's analytic benefit crosses zero at roughly **7.4% false positives**. A rule intended to protect outsiders can become costly over-refusal. This is a model-specific threshold, not a recommended deployment setting.

![Imperfect evidence results](results/robustness/comparison.png)

For collaborators: start with [related work and contribution paths](docs/RELATED_WORK.md), the [model assumptions](MODEL.md), or an [independent reproduction](https://github.com/melissagrail/coordination-repair-lab/issues/new?template=reproduction.md). Counterexamples are welcome. The [v0.1.0 tag](https://github.com/melissagrail/coordination-repair-lab/tree/v0.1.0) preserves the original release.
...
python3 robustness.py --seeds 20 --opportunities 1000 --out results/robustness
python3 verify_results.py
```

To independently regenerate and compare both complete studies, run `python3 verify_results.py --rerun`. For a small smoke run, use `python3 robustness.py --quick --seeds 2 --opportunities 100 --out /tmp/repair-smoke`; it is clearly marked as a partial study.

The checked-in baseline [report](results/REPORT.md) is generated by that command. The [CSV](results/runs.csv) contains every run, and the [manifest](results/manifest.json) records the parameters, policy definitions, seeds, source hash, and hashes of the event tapes. Re-running the same source with the same Python version should reproduce these files byte for byte.
...
## Original baseline findings (false positives disabled)
...
These are mean social-net units per opportunity, not real-world performance estimates. Bounded losses favor naive trust; the baseline's one-sided evidence favors retry and screening. The new study explicitly tests false positives. The point is to expose such dependencies, not conceal them.
...
3. Extend the new imperfect-evidence model to truthful allegations, calibrated uncertainty, heterogeneous loss sizes, or disagreement about externality estimates.
...
The initial implementation and studies were produced with AI assistance. Automated tests and analytic checks do not constitute human peer review or independent empirical validation.

MIT licensed. No telemetry or external simulation dependencies. The optional figures use Matplotlib; regenerate the new one with `python3 plot_robustness.py`. Tagged releases include [citation metadata](CITATION.cff) and a [changelog](CHANGELOG.md).
